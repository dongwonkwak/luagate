...
export function registerPolicyTools(server, client) {
    // luagate_get_policies — 정책 YAML + ETag 조회
    server.tool("luagate_get_policies", "현재 정책 YAML과 ETag(source_version)을 조회합니다. 정책 수정 전 반드시 호출하여 expected_source_version을 확보하세요.", {}, async () => {
        try {
    ...
    server.tool("luagate_get_policy_versions", "현재 정책 버전 스냅샷을 조회합니다 (source_version, active_http_version, active_stream_version).", {}, async () => {
        try {
