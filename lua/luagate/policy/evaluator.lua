...
local _cached_version = nil -- "<http_version>|<stream_version>" composite cache key
...
--- Load and decode a policy blob for a specific active version.
-- @param dict    table   ngx.shared.luagate_policy-compatible dict
-- @param version string  Active version hash
-- @return table|nil, string|nil
local function load_policy_blob(dict, version)
  local blob_key = "policy:" .. version .. ":blob"
  local policy_json = dict:get(blob_key)
  if not policy_json then
    return nil, "policy blob not found for version: " .. version
  end

  local ok, policy = pcall(require("cjson").decode, policy_json)
  if not ok or type(policy) ~= "table" then
    return nil, "failed to decode policy blob for version " .. version .. ": " .. tostring(policy)
  end
...
function _M.get_policy()
  ...
  if http_ver then
    http_policy, policy = load_policy_blob(dict, http_ver)
    if not http_policy then
      ngx.log(ngx.ERR, "[luagate] ", policy)
      return _cached_policy -- last-known-good
    end
  end

  if stream_ver and stream_ver ~= http_ver then
    stream_policy, policy = load_policy_blob(dict, stream_ver)
    if not stream_policy then
      ngx.log(ngx.ERR, "[luagate] ", policy)
      return _cached_policy -- last-known-good
    end
  ...
  -- Pre-compile sorted rule lists for the effective subsystem view.
