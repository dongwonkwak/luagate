# Codex Review: DON-216 — Admin API PUT — UTF-8 only + BOM 미허용 요청 검증 구현 (code)
...
```
lua/luagate/admin/policies.lua
tests/unit/admin/policies_spec.lua
```

## 관련 스펙 문서

- `docs/spec/admin-api.md` §6.5 — PUT /api/v1/policies 요청 제한 (UTF-8 only, BOM 미허용)

## Acceptance Criteria

1. body 앞 3바이트가 UTF-8 BOM (0xEF, 0xBB, 0xBF)이면 422 반환
2. Content-Type charset 파라미터가 UTF-8이 아니면 422 반환
3. charset 미명시 시 UTF-8로 간주하여 통과
4. dry_run=true에서도 동일하게 검증 적용
...
결과 파일 경로: .claude/reviews/DON-216-code-result.md
