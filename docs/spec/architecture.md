...
| `luagate_ratelimit` | Data plane HTTP 규칙별 sliding window rate limit 카운터 ([ADR-012](../design/adr/ADR-012-http-data-plane-rate-limiting.md)) | 단순 카운터: `rl:<scope_key>:<slot>` | 각 worker (incr) | 키 단위 |
...
| logging 실패 (직렬화) | pre-commit: fail-closed, post-commit: warn-only | ADR-004: pre-commit audit 실패 → 거부. post-commit audit 실패 → 경고. 디스크 I/O는 Nginx에 위임 |
